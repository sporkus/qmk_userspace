...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
  ...
  if (achordion_state == STATE_RELEASED) {
    if (is_tap_hold && record->tap.count == 0 && record->event.pressed &&
      ...
      if (timeout > 0) {
        ...
        uprintf("Achordion: Key 0x%04X pressed.%s\n", keycode,
  ...
  if (achordion_state == STATE_UNSETTLED && record->event.pressed) {
    ...
    if (!is_key_event || (is_tap_hold && record->tap.count == 0) ||
      ...
      uprintln("Achordion: Plumbing hold press.");
      settle_as_hold();
    } else {
      clear_eager_mods();  // Clear in case eager mods were set.

      uprintln("Achordion: Plumbing tap press.");
      ...
      uprintln("Achordion: Plumbing tap release.");
      tap_hold_record.event.pressed = false;
...
void achordion_task(void) {
  if (achordion_state == STATE_UNSETTLED &&
    ...
    uprintln("Achordion: Timeout. Plumbing hold press.");
...
bool achordion_opposite_hands(const keyrecord_t* tap_hold_record,
  ...
  bool is_opposite_hand = on_left_hand(tap_hold_record->event.key) !=
         on_left_hand(other_record->event.key);

  if (is_opposite_hand) {
      uprintln("Achordion: opposite hands");
  } else {
      uprintln("Achordion: same hand");
  }

  return is_opposite_hand;
}
...
__attribute__((weak)) bool achordion_chord(uint16_t tap_hold_keycode,
    ...
    if (other_record -> event.key.row + 1 == MATRIX_ROWS  ) {
        uprintln("Achordion: opposite hands ignored. The other key is on the last row.");
        return true;
    }
